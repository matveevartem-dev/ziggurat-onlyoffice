...
CA_CERTIFICATES_PATH=${CA_CERTIFICATES_PATH:-${SSL_CERTIFICATES_DIR}/}
...
update_welcome_page() {
  ...
  if [[ -e $WELCOME_PAGE ]]; then
    DOCKER_CONTAINER_ID=$(basename $(cat /proc/self/cgroup))
...
# add selfsigned certs and add ${SITE_NAME} to /etc/hosts
if [[ -n "$APP_ENV" && "$APP_ENV" == "dev" ]]; then
  mkdir -p /var/www/onlyoffice/Data/certs/
  openssl req -x509 -newkey rsa:4096 \
    -keyout "/var/www/onlyoffice/Data/certs/${ONLYOFFICE_NAME}.key" \
    -out "/var/www/onlyoffice/Data/certs/${ONLYOFFICE_NAME}.crt" \
    -sha256 -days 3650 -nodes \
    -subj "/C=RU/ST=Moscow/L=Moscow/O=PerevodPravo/OU=PerevodPravo/CN=${ONLYOFFICE_NAME}"
  echo ${LOCAL_INTERFACE_IP} ${SITE_NAME} >> /etc/hosts
fi
