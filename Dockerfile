...
ENV ONLYOFFICE_NAME=${ONLYOFFICE_NAME}
ENV LOCAL_INTERFACE_IP=${LOCAL_INTERFACE_IP}

#ENV SSL_SELFSIGNED_CERTIFICATE=true
...
ENV COMPANY_NAME=$COMPANY_NAME \
    ...
    #SSL_SELFSIGNED_CERTIFICATE=true \
...
#RUN find /var/www/onlyoffice/documentserver -name sdk-all-min.js -exec sed -E -i 's|(function +\w+\(\w+\) *\{ *function +\w+\(\)) *\{ *(\w+)\.open\((\w+),(\w+),(\w+)\);|\1{\nif (\4 \&\& \4.length > 5) {if (\4.substring(0, 5) == "http:") {\4 = \4.replace("http://", "https://");}};\n\2.open(\3,\4,\5);\n|' '{}' ';'

RUN mkdir -p /var/www/onlyoffice/Data/certs

RUN echo $ONLYOFFICE_NAME
